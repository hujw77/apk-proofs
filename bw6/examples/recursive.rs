...
fn main() {
    ...
    println!(
        "pks_comm_x: {}",
        genesis_validator_set_commitment.pks_comm.0
    );
    println!(
        "pks_comm_y: {}",
        genesis_validator_set_commitment.pks_comm.1
    );
    end_timer!(t_genesis);
    ...
    for era in 1..=n_eras {
        ...
        println!("public_input.apk: {}", public_input.apk);
        println!("public_input.bitmask: {:?}", public_input.bitmask.as_ref());
        println!(
            "public_input.bitmask.size: {:?}",
            public_input.bitmask.size()
        );
        use ark_ff::biginteger::{BigInteger, BigInteger256};
        let b = BigInteger256::new(public_input.bitmask.as_ref().try_into().unwrap());
        println!(
            "public_input.bitmask.be: {:?}",
            array_bytes::bytes2hex("0x", b.to_bytes_be())
        );

        println!("aggregate_signature: {}", aggregate_signature.as_ref());
        println!(
            "new_validator_set_commitment.pks_comm.0: {}",
            new_validator_set_commitment.pks_comm.0
        );
        println!(
            "new_validator_set_commitment.pks_comm.1: {}",
            new_validator_set_commitment.pks_comm.1
        );
        println!(
            "new_validator_set_commitment.log_domain_size: {}",
            new_validator_set_commitment.log_domain_size
        );
