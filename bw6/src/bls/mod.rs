...
impl PublicKey {
    ...
    pub fn verify(&self, signature: &Signature, message: &G2Projective) -> bool {
        // println!("neg_gen: {:?}", G1Affine::generator().neg());
        // println!("public_key: {:?}", &self.0.into_affine());
        // println!("signature: {:?}", signature.as_ref().into_affine());
        println!("message: {:?}", message.into_affine());
