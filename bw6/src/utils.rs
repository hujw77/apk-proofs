...
pub fn barycentric_eval_at<F: FftField>(
    z: F,
    evals: &Vec<F>,
...
pub fn barycentric_eval_binary_at<F: FftField>(
    z: F,
    evals: &Bitmask,
    domain: Radix2EvaluationDomain<F>,
) -> F {
    ...
    let mut acc = z;
    // println!("acc: {acc}");
    // println!("evals.to_bits: {:?}", evals.to_bits());
    for b in evals.to_bits() {
        if b {
            // println!("acc-1: {}", acc - F::one());
            li_inv.push(acc - F::one());
        }
        acc *= domain.group_gen_inv;
    }
...
pub fn lagrange_evaluations<F: FftField>(
    z: F,
    domain: Radix2EvaluationDomain<F>,
...
mod tests {
    ...
    pub fn test_barycentric_eval() {
        ...
        assert_eq!(
            barycentric_eval_at(z, &bits_as_field_elements, domain),
            bits_poly_at_z
        );
        assert_eq!(
            barycentric_eval_binary_at(z, &bitmask, domain),
            bits_poly_at_z
        );
    }
